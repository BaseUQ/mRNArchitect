                 ...
                 box(
                   ...
                   tags$div(
                     ...
                     tags$h4(tags$b("Example: "), "For guidance on how to design an mRNA, please see the step-by-step example ", tags$a(href = "https://basefacility.org.au/wp-content/uploads/2024/12/mRNArchitect_Example.pdf", 
                                                                                                                                        "here", target = "_blank")), 
                     tags$h4(tags$b("Sequences: "), "Please find useful sequences (promoters, UTRs etc.) ", tags$a(href = "https://basefacility.org.au/wp-content/uploads/2024/12/mRNArchitect_ExampleSequences.txt", 
                                                                                                                   "here", target = "_blank"))
                   )
                 ),
                 box(
...
server <- function(input, output, session) {
  ...
  # Reactive values to store separate sequences
  sequence_store <- reactiveValues(
    nucleotide = "",
    amino_acid = ""
  )
  
  observe({
    # Store the input based on the current sequence type
    if (input$sequence_type == "Nucleotide") {
      sequence_store$nucleotide <- input$sequenceText
    } else {
      sequence_store$amino_acid <- input$sequenceText
    }
  })
  
  observeEvent(input$sequence_type, {
    if (input$sequence_type == "Amino acid") {
      # Switch to Amino acid, load stored amino acid sequence
      updateTextAreaInput(session, "sequenceText", value = sequence_store$amino_acid)
    } else {
      # Switch to Nucleotide, load stored nucleotide sequence
      updateTextAreaInput(session, "sequenceText", value = sequence_store$nucleotide)
    }
  })
  ...
  observe({
    ...
    updateSelectizeInput(session, "restrictionSites",
  ...
  output$restrictionSites <- renderUI({
    restriction_sites <- names(rest_dict)
    selectizeInput("restrictionSites", "Avoid cut sites",
                   choices = NULL,  
                   multiple = TRUE)
  })
  ...
  observeEvent(input$runButton, {
    ...
    if (valid) {
      ...
      # Check for start codon (ATG) and stop codons (TAA, TAG, TGA)
      start_codon_present <- grepl("^ATG", nucleotide_sequence)
      stop_codon_present <- grepl("(TAA|TAG|TGA)$", nucleotide_sequence)
      # Check if either start or stop codon is missing
      if (!start_codon_present || !stop_codon_present) {
        # Display a warning modal using the custom function
        if (!start_codon_present && !stop_codon_present) {
      ...
      # Check for codons GGC and AUG in the 5'UTR
      #UTR5 <- ifelse(input$UTR5 != "", toupper(input$UTR5), "")
      ...
      print(paste0("Job ID: ", job_id, "; Optimisation start: ", start_timestamp, "; Input sequence: ", nucleotide_sequence))
      ...
      # Extract and combine values from each data frame
      values_list <- lapply(results_list, extract_values)
      
      numeric_values_list <- lapply(values_list, function(x) as.numeric(x))
      
      # Convert the list of values to a matrix for easier sorting
      values_matrix <- do.call(rbind, numeric_values_list)
      
      # Create a sorting order based on GC ratio (desc), CAI (desc), and CDS MFE (asc)
      sorting_order <- order(-values_matrix[, 1],  # Highest GC ratio
                             -values_matrix[, 2],  # Highest CAI
                             values_matrix[, 3])   # Lowest CDS MFE
      
      # Rearrange the data frames in the list based on the sorting order
      ...
      print(paste0("Job ID: ", job_id, ", Total execution time: ", round(execution_time, digits = 3), " seconds"))
  ...
  output$downloadExampleSeq <- downloadHandler(
    filename = function() {
      "example_sequence.fa"
    },
    content = function(file) {
      file.copy("data/example_sequence.fa", file)
    }
  )
  
  output$downloadExampleResults <- downloadHandler(
    filename = function() {
      "example_results.csv"
    },
    content = function(file) {
      file.copy("data/example_results.csv", file)
    }
  )
}
