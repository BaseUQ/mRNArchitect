...
import { searchOrganisms, type Organism } from "~/api";
...
export const OrganismSelect = ({ label, value, onChange }: OrganismSelectProps) => {
  ...
  useEffect(() => {
    if (searchValue) {
      setIsLoading(true);
      searchOrganisms({ terms: searchValue }).then(({ organisms }) => setOrganisms(organisms)).finally(() => setIsLoading(false));
    } else {
  ...
  const options = useMemo(() => {
    ...
    if (!searchValue) {
      return <Combobox.Group label="Common organisms">{options}</Combobox.Group>;
    }

    return <Combobox.Options>{options}</Combobox.Options>;
  }, [organisms, searchValue, value]
  );
